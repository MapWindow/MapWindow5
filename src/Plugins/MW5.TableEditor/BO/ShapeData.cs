...
namespace MW5.Plugins.TableEditor.BO
{
  ...
  /// </summary>
  public class ShapeData : MapWinGIS.ICallback
  {
    ...
    {
      String[] options = joinOptions.Split(new char[] { ';' });
      ...
      if (datatChangedData != null)
      {
        for (int i = 0; i < datatChangedData.Rows.Count; i++)
        {
          for (int j = 0; j < sf.NumFields; j++)
          {
            ...
            {
              MessageBox.Show(string.Format("Error in EditCellValue: {0}", sf.get_ErrorMsg(sf.LastErrorCode)));
            }
          }
        }
      }
    ...
    {
      for (int newCol = sf.NumFields + 1; newCol < dataTable.Columns.Count; newCol++)
      {
    ...
    public void AddField(DataColumn column)
    {
      ...
      FieldType fieldType = GetMapWindowFieldType(column.DataType);
      int precision = GetPrecision(column);
      int width = GetWidth(column);

      int fieldIndexNew = sf.EditAddField(name, fieldType, precision, width);
    }
    ...
    private DataTable CreateTable()
    {
      ...
      for (int i = 0; i < sf.NumFields; i++)
      {
        ...
        dataColumn.ColumnName = sf.Table.get_FieldIsJoined(i) ? field.Name : GetValidFieldName(field.Name, dataTable);
