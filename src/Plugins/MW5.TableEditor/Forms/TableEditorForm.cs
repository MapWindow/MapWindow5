...
namespace MW5.Plugins.TableEditor.Forms
{
  ...
  {
    /// <summary>
    /// The fastloa d_ amount.
    /// </summary>
    private const int FastloadAmount = 1000;

    /// <summary>
    ///   The BOMapWindow-object
    /// </summary>
    ...
    /// <summary>
    /// The is structure changed.
    /// </summary>
    private bool _isStructureChanged;

    /// <summary>
    ///   The selected columnindex
    /// </summary>
    ...
    private void EnableButtons()
    {
      ...
      if (ShapefileWrapper.ShapeFile.GeoProjection.IsEmpty && ShapefileWrapper.ShapeFile.Projection == string.Empty)
      {
    ...
    private void OpenUpdateMeasurmentsForm()
    {
      ...
      if (ShapefileWrapper.ShapeFile.ShapefileType == ShpfileType.SHP_POINT
          ...
          || ShapefileWrapper.ShapeFile.ShapefileType == ShpfileType.SHP_POINTZ)
      {
        MessageBox.Show(@"You've opened a point shapefile. The measurements cannot be calculated for point shapefiles");
        return;
      }
    ...
    private void UpdateFromQuerySelection(SelectionOperation operation, List<int> selectedIds)
    {
      switch (operation)
      {
        ...
        case SelectionOperation.Add:
          foreach (var row in
            TableEditorDataGrid.Rows.Cast<DataGridViewRow>().Where(row => selectedIds.Exists(elm => elm == Convert.ToInt32(row.Cells[0].Value))))
          {
            row.Selected = true;
          }
    ...
    private void BtnGetAllClick(object sender, EventArgs e)
    {
      ...
      // PM Added: Select rows of previously selected shapes:
      System.Diagnostics.Debug.WriteLine("Num selected: " + ShapefileWrapper.ShapeFile.NumSelected);
      SetSelected();
    ...
    private void MnuCopyShapeIDsClick(object sender, EventArgs e)
    {
      ...
      FormUtils.InputBox(
        "Assign Feature ID to a field", "Enter the name of the target field  (Data will be overwritten)", ref result);
